angular.module('tellefile.filters', ['tellefile.i18n'])
...
.filter('userStatus', function($filter, _){
	...
	return function (user){
		var statusType = user && user.status && 
			user.status._;
		if (!statusType) {
			statusType = user && user.pFlags;
		}

		switch (statusType) {
			case 'userStatusOnling' :
	        ...
	        case 'userStatusEmpty' : 
	        	default: return _('user_status_long_ago');
		}
	}
})
